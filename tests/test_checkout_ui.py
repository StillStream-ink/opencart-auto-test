...
class TestCheckoutUI:
    ...
    def test_checkout_success(self, page: Page):
        """正向结算：登录 → 添加iPhone商品 → 结算选择地址、配送、支付
        备注：受OpenCart本地运费/地区配置限制，暂时跳过最终提交订单操作
        """
        ...
        with allure.step("2.搜索商品：iPhone，进入商品详情页"):
            cart_page.search_product("iPhone")
            cart_page.open_product_detail("iPhone")
